...
import {
  ...
  describeCertifiedLibraryStatus
} from '../core/certified-cycle-controller.js';
import { generateDiverseHamiltonianCycles, type CycleLibrary } from '../core/cycle-library.js';
...
import { ValidationWorkerClient, type ValidationTaskState } from './validation-worker-client.js';
...
type Screen = 'menu' | 'play-setup' | 'game' | 'designer' | 'library' | 'scores' | 'settings';
type DesignerTool = 'wall' | 'erase' | 'spawn' | 'portal';
type PlayMode = 'human' | 'ai';

interface DesignerState {
  draft: EditableMapDraft;
  selectedTool: DesignerTool;
  pendingPortalAnchor: Coord | null;
  validation: ValidationTaskState;
...
function draftFromSavedMap(map: SavedMap): EditableMapDraft {
  return {
    id: map.id,
    name: map.name,
    width: map.width,
...
function makeValidationState(result: MapValidationResult | null, status: ValidationTaskState['status'], expansions = 0): ValidationTaskState {
  return {
    status,
    result,
    expansions
...
function createIllegalDesignerResult(draft: EditableMapDraft, messageText: string): MapValidationResult {
...
function coordEquals(left: Coord | null, right: Coord | null): boolean {
  return !!left && !!right && left.x === right.x && left.y === right.y;
}

export class SnakeApp {
  private readonly mapStore = new MapStore();
  private readonly scoreStore = new HighScoreStore();
  private readonly settingsStore = new SettingsStore();
  private readonly validationClient = new ValidationWorkerClient();
  private readonly certifiedCycleLibraries = new Map<string, CycleLibrary>();

  private screen: Screen = 'menu';
  private playMode: PlayMode = 'human';
  private aiStrategy: AiStrategyName = 'greedy';
  private maps: SavedMap[] = [];
  ...
  private savedPlacement: number | null = null;
  private designerValidationDebounce: number | null = null;
  private validationNonce = 0;

  private designer: DesignerState = {
  ...
  private get selectedMap(): SavedMap | null {
  ...
  private ensureCertifiedCycleLibrary(map: SavedMap): CycleLibrary {
  ...
  private handleInput = (event: Event): void => {
    ...
    switch (field) {
      case 'selected-map':
        ...
        return;
      case 'ai-strategy':
        this.aiStrategy = target.value as AiStrategyName;
        this.render();
        return;
      case 'score-filter':
        this.settings = {
          ...this.settings,
          scoreFilter: target.value as SettingsData['scoreFilter']
        };
      ...
      case 'designer-height':
        ...
        return;
      case 'designer-tool':
        this.designer.selectedTool = target.value as DesignerTool;
        this.render();
        return;
  ...
  private startSelectedMapGame(): void {
  ...
  private tickGame(): void {
    ...
    let nextDirection: Direction | null = this.requestedDirection;
  ...
  private loadMapIntoDesigner(map: SavedMap): void {
    this.designer = {
      ...this.designer,
  ...
  private scheduleDesignerValidation(): void {
    ...
    this.designerValidationDebounce = window.setTimeout(async () => {
      if (this.designer.pendingPortalAnchor) {
        this.designer.validation = makeValidationState(
          createIllegalDesignerResult(this.designer.draft, 'Portals must be linked in pairs before the map can be legal.'),
          'invalid'
        );
        this.render();
        return;
      ...
      try {
        ...
        this.designer.validation = makeValidationState(
          createIllegalDesignerResult(this.designer.draft, 'Validation failed unexpectedly.'),
          'invalid'
        );
        this.render();
      }
  ...
  private handleDesignerCanvasClick(cell: Coord): void {
  ...
  private renderPlaySetup(): string {
  ...
  private renderGame(): string {
    ...
    const certifiedLibraryStatus =
      state.aiStrategy === 'certified-hamiltonian'
  ...
  private mapOptionsHtml(): string {
    return this.maps
      .map(
        (map) => `<option value="${map.id}" ${map.id === this.selectedMapId ? 'selected' : ''}>${this.escapeHtml(map.name)}</option>`
      )
      .join('');
  }

  private renderScoreEntries(entries: ReturnType<HighScoreStore['listAll']>): string {
    if (entries.length === 0) {
  ...
  private renderDetailedScores(entries: ReturnType<HighScoreStore['listAll']>): string {
    if (entries.length === 0) {
      return '<p>No scores recorded yet.</p>';
    }
    return `
  ...
  private renderValidationReasons(): string {
  ...
  private toolButton(tool: DesignerTool, label: string): string {
    return `
  ...
  private afterRender(): void {
    if (this.screen === 'menu' || this.screen === 'play-setup' || this.screen === 'library') {
      const previewCanvas = this.root.querySelector<HTMLCanvasElement>('#preview-canvas');
    ...
    if (this.screen === 'designer') {
      const canvas = this.root.querySelector<HTMLCanvasElement>('#designer-canvas');
      if (canvas) {
