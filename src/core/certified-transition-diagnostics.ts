...
import { explainLockedCertificateFailure, validLockedCertificate } from './hamiltonian-certificate.js';
import type { Direction, GameState, HamiltonianCycle, NodeId } from './types.js';

export interface CertifiedTransitionDiagnosticsOptions {
  slack?: number;
  maxPaths?: number;
...
export interface CertifiedTransitionFailureReasons {
...
export interface CertifiedTransitionTargetDiagnostics {
  targetCycleId: string;
  source: CycleLibraryEntry['source'];
  archetypeName: string;
  isCurrentLockedCycle: boolean;
  shortestDistanceToApple: number | null;
  ...
  targetPathLenAfterTransition: number | null;
  targetScoreAfterTransition: number | null;
  failureReasons: CertifiedTransitionFailureReasons;
  lockedCertificateFailures: string[];
}

export interface CertifiedTransitionDiagnostics {
  snakeLength: number;
  applePosition: NodeId | null;
  head: NodeId | null;
  tail: NodeId | null;
  librarySize: number;
  targetCyclesChecked: number;
  targetsWithSuccessfulTransition: number;
  totalSuccessfulTransitionPaths: number;
  bestTargetCycleId: string | null;
  bestPathLength: number | null;
  bestPath: Direction[] | null;
  currentLockedCycleId: string | null;
  shortestDistanceToApple: number | null;
  pathsGenerated: number;
  budgetExceeded: boolean;
  targets: CertifiedTransitionTargetDiagnostics[];
}

interface GeneratedPathSet {
  paths: Direction[][];
  shortestDistance: number | null;
  budgetExceeded: boolean;
}

const DEFAULT_OPTIONS: Required<CertifiedTransitionDiagnosticsOptions> = {
  slack: 6,
  maxPaths: 64,
...
function resolvedOptions(options: CertifiedTransitionDiagnosticsOptions): Required<CertifiedTransitionDiagnosticsOptions> {
  return {
    ...DEFAULT_OPTIONS,
    ...options
  };
...
function currentLockedCycle(state: GameState): HamiltonianCycle {
  return state.lockedHamiltonianCycle ?? state.map.hamiltonianCycle;
}

function currentLockedSignature(state: GameState): string {
  return cycleEdgeSignature(currentLockedCycle(state));
}

function isCurrentLockedEntry(state: GameState, entry: CycleLibraryEntry): boolean {
...
function shortestGraphDistance(state: GameState, from: NodeId, to: NodeId): number | null {
  const graph = hydrateGraph(state.map.graph);
  const queue: Array<{ node: NodeId; distance: number }> = [{ node: from, distance: 0 }];
...
function sortedOutgoingEdges(state: GameState, nodeId: NodeId) {
  const graph = hydrateGraph(state.map.graph);
...
export function generateCandidatePathsToApple(
  state: GameState,
  options: CertifiedTransitionDiagnosticsOptions = {}
): GeneratedPathSet {
...
function simulatePathUntilApple(state: GameState, path: Direction[]): {
  status: 'apple-eaten' | 'collision-before-apple' | 'apple-not-reached' | 'simulation-ended-before-apple';
  state: GameState;
} {
...
function createFailureReasons(budgetExceeded: boolean): CertifiedTransitionFailureReasons {
  return {
    noPathGenerated: 0,
...
function analyzeTargetCycle(
  state: GameState,
  entry: CycleLibraryEntry,
  paths: Direction[][],
  shortestDistance: number | null,
  budgetExceeded: boolean
): CertifiedTransitionTargetDiagnostics {
  const failureReasons = createFailureReasons(budgetExceeded);
  const lockedCertificateFailures = new Set<string>();
  let safePathsToApple = 0;
  let successfulTransitionPaths = 0;
  let bestSuccessfulPath: Direction[] | null = null;
  let bestTransitionState: GameState | null = null;
...
export function analyzeCertifiedTransitionTargets(
  state: GameState,
  cycleLibrary: CycleLibrary,
  options: CertifiedTransitionDiagnosticsOptions = {}
): CertifiedTransitionDiagnostics {
  const generated = generateCandidatePathsToApple(state, options);
  const targets = cycleLibrary.entries.map((entry) => analyzeTargetCycle(
    state,
    entry,
    generated.paths,
    generated.shortestDistance,
    generated.budgetExceeded
  ));
