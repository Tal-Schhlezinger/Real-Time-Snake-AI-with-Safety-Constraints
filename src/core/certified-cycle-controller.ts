...
} from './hamiltonian-certificate.js';
import {
...
import {
  generateRectanglePatchMutationCandidatesFromRectangles,
  generateTargetedRectangles,
  type TargetedRectangleSearchMode
} from './head-apple-rectangle-diagnostics.js';
import type { AiDecision, CertifiedTransitionPlan, Direction, GameState, HamiltonianCycle, NodeId } from './types.js';
import type { RectanglePatchRect } from './two-terminal-patch-mutation.js';

export interface CertifiedCycleSelectionOptions {
  scoreWeights?: CycleScoreWeights;
  minimumPathImprovement?: number;
  enablePatchMutation?: boolean;
  maxPatchRectsScanned?: number;
  maxPatchCandidates?: number;
  maxPatchArea?: number;
  maxPatchWidth?: number;
  maxPatchHeight?: number;
  maxTransitionPathsPerCandidate?: number;
  maxTransitionSlack?: number;
  enableV2PatchMutation?: boolean;
  maxV2FillRatio?: number;
  maxV2RectsScanned?: number;
  maxV2Candidates?: number;
  maxV2PatchArea?: number;
  maxV2TransitionPathsPerCandidate?: number;
  maxV2TransitionSlack?: number;
  maxV2TransitionPathLength?: number;
  maxV2TransitionSearchStates?: number;
  maxV2SolverExpansions?: number;
  maxV2PathCoversPerPatch?: number;
  enablePatchMutationCandidateCache?: boolean;
  patchMutationCandidateCache?: PatchMutationCandidateCache;
  patchRectangleSearchMode?: 'broad' | 'head-apple' | 'arc-chunk' | 'combined-targeted' | 'targeted-then-broad-fallback';
  arcChunkSize?: number;
  arcChunkStride?: number;
  arcGrowShrinkRadius?: number;
  maxTargetedRectangles?: number;
  fallbackToBroadIfNoCandidates?: boolean;
  transitionPrefilterMode?: 'none' | 'cheap-score' | 'body-order-compatibility' | 'combined';
...
export type CertifiedSelectedCandidateSource = 'current-cycle' | 'library' | 'transition' | 'v1-patch' | 'v2-patch';

export interface CertifiedRuntimeSwitchingDiagnostics {
  applesEaten: number;
  switchAttempts: number;
  successfulSwitches: number;
  candidateCyclesChecked: number;
  candidatesPassingProofGate: number;
  candidatesRejectedByGraphValidity: number;
  candidatesRejectedByLockedCertificate: number;
  candidatesRejectedByAppleForward: number;
  oldPathLenBeforeSwitch: number[];
  newPathLenAfterSwitch: number[];
  averagePathLenImprovement: number | null;
  oldCycleKept: number;
  noValidSwitchExists: number;
  proofValidButNotImproving: number;
  proofValidAndPathLenImproving: number;
  proofValidAndScoreImproving: number;
  bestObservedPathLenImprovement: number | null;
  bestObservedScoreImprovement: number | null;
  averageBestCandidatePathLenDelta: number | null;
  averageCurrentPathLen: number | null;
  averageBestCandidatePathLen: number | null;
  patchMutationAttempted: number;
  patchRectsScanned: number;
  patchGraphValidCandidates: number;
  patchSnakeUsableCandidates: number;
  patchImmediateLockedCandidates: number;
  patchNonImmediateCandidates: number;
  patchTransitionCandidatesAfterPrefilter: number;
  patchTransitionCandidatesSkippedByPrefilter: number;
  patchTransitionSearchesStarted: number;
  patchTransitionSearchesSucceeded: number;
  patchTransitionReachableCandidates: number;
  patchRejectedByLockedCertificate: number;
  patchImprovingCandidates: number;
  patchSelectedCandidates: number;
  patchRejectedNoImprovement: number;
  patchRejectedBudget: number;
  patchImmediateLockedSelectedWithoutTransition: number;
  bestPatchImprovement: number | null;
  patchGenerationMs: number;
  ...
  v1ScoringMs: number;
  v2PatchAttempted: number;
  v2RectsScanned: number;
  v2RawCandidatesGenerated: number;
  v2GraphValidCandidates: number;
  v2SnakeUsableCandidates: number;
  v2ImmediateLockedCandidates: number;
  v2NonImmediateCandidates: number;
  v2TransitionCandidatesAfterPrefilter: number;
  v2TransitionCandidatesSkippedByPrefilter: number;
  v2TransitionSearchesStarted: number;
  v2TransitionSearchesSucceeded: number;
  v2TransitionReachableCandidates: number;
  v2RejectedByLockedCertificate: number;
  v2ImprovingCandidates: number;
  v2SelectedCandidates: number;
  v2ImmediateLockedSelections: number;
  v2TransitionSelections: number;
  v2RejectedNoImprovement: number;
  v2RejectedBudget: number;
  v2ImmediateLockedSelectedWithoutTransition: number;
  bestV2Improvement: number | null;
  ...
  v2ScoringMs: number;
  rectangleSearchMode: CertifiedCycleSelectionOptions['patchRectangleSearchMode'];
  targetedRectanglesGenerated: number;
  targetedRectanglesUsed: number;
  broadFallbackUsed: number;
  arcChunkRectangles: number;
  headAppleRectangles: number;
  candidateYieldPerRectangle: number | null;
  improvingYieldPerRectangle: number | null;
  selectedCandidateSource: CertifiedSelectedCandidateSource | null;
  switchAttemptSummaries: CertifiedSwitchAttemptDiagnostic[];
}

export interface CertifiedProofValidCandidateDiagnostic {
  candidateId: string;
  candidatePathLen: number | null;
  candidateScore: number;
  pathLenDelta: number | null;
  scoreDelta: number | null;
  rejectedBecauseNotImprovingPathLen: boolean;
  rejectedBecauseScoreComparatorWorse: boolean;
}

export interface CertifiedSwitchCandidateSummary {
  candidateId: string;
  pathLen: number | null;
  score: number;
  pathLenDelta: number | null;
...
export type CertifiedSwitchDecisionReason =
  | 'no-proof-valid-candidates'
  | 'no-pathLen-improvement'
  | 'score-not-better'
  | 'kept-current-by-tie-break'
  | 'selected-switch';

export interface CertifiedSwitchAttemptDiagnostic {
  currentLockedCycleId: string | null;
  currentCyclePathLen: number | null;
  currentCycleScore: number;
  candidatesChecked: number;
  candidatesPassingGraphValidity: number;
  candidatesPassingLockedCertificate: number;
  candidatesPassingAppleForward: number;
  proofValidCandidates: CertifiedProofValidCandidateDiagnostic[];
  bestProofValidCandidateByPathLen: CertifiedSwitchCandidateSummary | null;
  bestProofValidCandidateByScore: CertifiedSwitchCandidateSummary | null;
  finalDecisionReason: CertifiedSwitchDecisionReason;
  selectedCycleId: string | null;
  selectedPathLen: number | null;
  selectedCandidateSource: CertifiedSelectedCandidateSource;
  patchMutationAttempted: boolean;
  patchRectsScanned: number;
  patchGraphValidCandidates: number;
  patchSnakeUsableCandidates: number;
  patchImmediateLockedCandidates: number;
  ...
  candidateYieldPerRectangle: number | null;
  improvingYieldPerRectangle: number | null;
  selectedCandidateUsedTransitionPlan: boolean;
}

export type CertifiedCandidateDecision =
  | 'skipped-current-cycle'
  | 'rejected graph-invalid'
  | 'rejected locked-certificate'
  | 'rejected appleForward'
  | 'rejected no pathLen improvement'
  | 'rejected score worse'
  | 'selected';

export interface CertifiedCandidateDebugEvaluation {
  candidateId: string;
  source: CycleLibraryEntry['source'];
  archetypeName: string;
  pathLen: number | null;
  score: number | null;
  graphValid: boolean;
  lockedCertificateValid: boolean;
  lockedCertificateFailure: string | null;
  appleForwardValid: boolean | null;
  pathLenDelta: number | null;
  scoreDelta: number | null;
  finalDecision: CertifiedCandidateDecision;
}

export interface CertifiedSwitchDebugSnapshot {
  currentLockedCycleId: string | null;
  currentCyclePathLen: number | null;
  currentCycleScore: number | null;
  finalDecisionReason: CertifiedSwitchDecisionReason;
  selectedCycleId: string | null;
  selectedPathLen: number | null;
  candidates: CertifiedCandidateDebugEvaluation[];
}

export function createCertifiedRuntimeSwitchingDiagnostics(): CertifiedRuntimeSwitchingDiagnostics {
  return {
    applesEaten: 0,
    switchAttempts: 0,
...
const DEFAULT_CERTIFIED_CYCLE_SELECTION_OPTIONS: Required<CertifiedCycleSelectionOptions> = {
...
function certifiedInvariantError(message: string): never {
...
function getCycleSuccessor(cycle: HamiltonianCycle, head: NodeId): NodeId | null {
...
function getStrictCycleDirection(state: GameState, cycle: HamiltonianCycle): Direction | null {
...
function currentLockedCycleSignature(state: GameState): string {
  return cycleEdgeSignature(getCertifiedLockedCycle(state));
}

function isSameAsLockedCycle(state: GameState, entry: CycleLibraryEntry): boolean {
...
export function canSwitchAndLock(state: GameState, candidateEntry: CycleLibraryEntry): boolean {
...
interface CertifiedSwitchSelectionStats {
  candidateCyclesChecked: number;
  candidatesPassingGraphValidity: number;
  candidatesPassingLockedCertificate: number;
  candidatesPassingAppleForward: number;
  ...
  candidatesRejectedByLockedCertificate: number;
  candidatesRejectedByAppleForward: number;
  hadAnyProofPassingCandidate: boolean;
  oldPathLenBeforeSwitch: number | null;
  newPathLenAfterSwitch: number | null;
  bestProofValidPathLenDelta: number | null;
  bestProofValidScoreDelta: number | null;
  bestProofValidCandidatePathLen: number | null;
  switchAttemptSummary: CertifiedSwitchAttemptDiagnostic | null;
  selectedCandidateSource: CertifiedSelectedCandidateSource;
  patchMutationAttempted: boolean;
  patchRectsScanned: number;
  patchGraphValidCandidates: number;
  ...
  patchImprovingCandidates: number;
  patchRejectedNoImprovement: number;
  patchRejectedBudget: boolean;
  patchImmediateLockedSelectedWithoutTransition: boolean;
  bestPatchImprovement: number | null;
  patchGenerationMs: number;
  v1CandidateCacheHits: number;
  v1CandidateCacheMisses: number;
  ...
  v2ImprovingCandidates: number;
  v2RejectedNoImprovement: number;
  v2RejectedBudget: boolean;
  v2ImmediateLockedSelectedWithoutTransition: boolean;
  bestV2Improvement: number | null;
  v2DetectionMs: number;
  v2PathCoverSolvingMs: number;
  v2SplicingValidationMs: number;
...
function createCertifiedSwitchSelectionStats(): CertifiedSwitchSelectionStats {
  return {
    candidateCyclesChecked: 0,
...
function rankCandidateEntries(
  ...
  candidates: CycleLibraryEntry[],
  options: Required<CertifiedCycleSelectionOptions>
): CycleLibraryEntry | null {
...
interface SelectedCertifiedCandidate {
  source: CertifiedSelectedCandidateSource;
  cycle: HamiltonianCycle;
  cycleId: string;
  pathLen: number | null;
  pathLenImprovement: number | null;
  score: number;
  transitionPlan: CertifiedTransitionPlan | null;
}

function patchCandidateCycleId(features: PatchMutationFeatures): string {
  return `v1-patch:${features.candidateId}`;
}

function v2PatchCandidateCycleId(features: V2FourExitMutationFeatures): string {
  return `v2-patch:${features.candidateId}`;
}

function buildPatchMutationClassificationOptions(options: Required<CertifiedCycleSelectionOptions>) {
  return {
...
function buildV2PatchMutationClassificationOptions(options: Required<CertifiedCycleSelectionOptions>) {
  return {
    maxWidth: options.maxPatchWidth,
    maxHeight: options.maxPatchHeight,
...
function buildExpectedHeadPathForTransition(state: GameState, directions: Direction[]): NodeId[] | null {
  const initialHead = state.snake.segments[0];
  if (!initialHead || directions.length === 0) {
    return null;
  }

  const expectedHeadPath: NodeId[] = [initialHead];
  let current = state;
...
function buildCertifiedTransitionPlanFromDirections(
  state: GameState,
  source: CertifiedTransitionPlan['source'],
  targetCycleId: string,
  targetCycle: HamiltonianCycle,
  directions: Direction[] | null
): CertifiedTransitionPlan | null {
...
function buildCertifiedTransitionPlan(
  state: GameState,
  rankedCandidate: RankedPatchMutationCandidate
): CertifiedTransitionPlan | null {
  if (rankedCandidate.features.usabilityMode !== 'transition-valid') {
    return null;
  }

  return buildCertifiedTransitionPlanFromDirections(
    state,
    'v1-patch',
    patchCandidateCycleId(rankedCandidate.features),
    rankedCandidate.candidate.cycle,
    rankedCandidate.classification.transitionPlanSummary?.bestSuccessfulPath ?? null
  );
}

function buildV2CertifiedTransitionPlan(
  state: GameState,
  ...
  return buildCertifiedTransitionPlanFromDirections(
    state,
    'v2-patch',
    v2PatchCandidateCycleId(rankedCandidate.features),
    rankedCandidate.candidate.cycle,
    rankedCandidate.classification.transitionPlanSummary?.bestSuccessfulPath ?? null
  );
}

type RuntimeTargetedRectanglePlan = {
  rectangles: RectanglePatchRect[] | null;
  arcChunkRectangles: number;
  headAppleRectangles: number;
};

type PatchMutationRankingWithCache = PatchMutationRankingResult & {
  cacheHit: boolean;
  cacheMiss: boolean;
};

type V2GenerationWithCache = {
  spliceDiagnostics: V2FourExitSpliceGenerationResult;
  cacheHit: boolean;
  cacheMiss: boolean;
};

function rankV1PatchCandidatesForRectangles(
  state: GameState,
  currentCycle: HamiltonianCycle,
  classificationOptions: ReturnType<typeof buildPatchMutationClassificationOptions>,
  options: Required<CertifiedCycleSelectionOptions>,
  rectangles: readonly RectanglePatchRect[] | null
): PatchMutationRankingWithCache {
  const cachedGeneration = options.enablePatchMutationCandidateCache
    ...
    : null;
  const generated = cachedGeneration?.result ?? (
    rectangles
...
function generateV2PatchCandidatesForRectangles(
  state: GameState,
  currentCycle: HamiltonianCycle,
  v2Options: ReturnType<typeof buildV2PatchMutationClassificationOptions>,
  options: Required<CertifiedCycleSelectionOptions>,
  rectangles: readonly RectanglePatchRect[] | null
): V2GenerationWithCache {
  const cachedGeneration = options.enablePatchMutationCandidateCache
    ? getOrCreateV2GraphCandidates({
      cache: options.patchMutationCandidateCache,
      mapId: state.map.id,
  ...
  return {
    spliceDiagnostics: cachedGeneration?.result ?? (
      rectangles
...
function buildRuntimeTargetedRectangles(
  state: GameState,
  options: Required<CertifiedCycleSelectionOptions>,
  mode: CertifiedCycleSelectionOptions['patchRectangleSearchMode']
): RuntimeTargetedRectanglePlan {
  if (!mode || mode === 'broad') {
    return {
      rectangles: null,
      arcChunkRectangles: 0,
  ...
  const targetedMode: TargetedRectangleSearchMode = mode === 'targeted-then-broad-fallback'
    ? 'arc-chunk'
    : mode;
  const rectangleOptions = {
    ...
    maxTargetRectangles: Math.min(
      options.maxTargetedRectangles,
      Number.isFinite(options.maxPatchRectsScanned) ? options.maxPatchRectsScanned : options.maxTargetedRectangles,
      Number.isFinite(options.maxV2RectsScanned) ? options.maxV2RectsScanned : options.maxTargetedRectangles
    ),
    arcChunkSize: options.arcChunkSize,
    arcChunkStride: options.arcChunkStride,
    arcGrowShrinkRadius: options.arcGrowShrinkRadius
...
function shouldUseBroadFallback(
...
function yieldPerRectangle(count: number, rectangles: number): number | null {
  return rectangles > 0 ? count / rectangles : null;
}

function selectBestPatchMutationCandidate(
  state: GameState,
  currentCycle: HamiltonianCycle,
  options: Required<CertifiedCycleSelectionOptions>
): {
  selected: SelectedCertifiedCandidate | null;
  stats: Pick<
    CertifiedSwitchSelectionStats,
    | 'patchMutationAttempted'
    | 'patchRectsScanned'
    | 'patchGraphValidCandidates'
    | 'patchSnakeUsableCandidates'
    | 'patchImmediateLockedCandidates'
    | 'patchNonImmediateCandidates'
    | 'patchTransitionCandidatesAfterPrefilter'
    | 'patchTransitionCandidatesSkippedByPrefilter'
    | 'patchTransitionSearchesStarted'
    | 'patchTransitionSearchesSucceeded'
    | 'patchTransitionReachableCandidates'
    | 'patchRejectedByLockedCertificate'
    | 'patchImprovingCandidates'
    | 'patchRejectedNoImprovement'
    | 'patchRejectedBudget'
    | 'patchImmediateLockedSelectedWithoutTransition'
    | 'bestPatchImprovement'
    | 'patchGenerationMs'
    ...
    | 'v1ScoringMs'
    | 'rectangleSearchMode'
    | 'targetedRectanglesGenerated'
    | 'targetedRectanglesUsed'
    | 'broadFallbackUsed'
    | 'arcChunkRectangles'
    | 'headAppleRectangles'
    | 'candidateYieldPerRectangle'
    | 'improvingYieldPerRectangle'
  >;
} {
  ...
  const improvingCandidates = ranking.rankedCandidates.filter(
    (candidate) => (candidate.features.pathLenImprovement ?? Number.NEGATIVE_INFINITY) >= options.minimumPathImprovement
  );
  let selected: SelectedCertifiedCandidate | null = null;
  ...
  return {
    ...
    stats: {
      ...
      candidateYieldPerRectangle: yieldPerRectangle(
        ranking.classificationDiagnostics.mutationDiagnostics.aggregate.graphValidCandidates,
        ranking.classificationDiagnostics.mutationDiagnostics.aggregate.patchesScanned
      ),
      improvingYieldPerRectangle: yieldPerRectangle(
        ranking.aggregate.improvingCandidates,
        ranking.classificationDiagnostics.mutationDiagnostics.aggregate.patchesScanned
      )
    }
  };
...
function selectBestV2PatchMutationCandidate(
  state: GameState,
  currentCycle: HamiltonianCycle,
  options: Required<CertifiedCycleSelectionOptions>
): {
  ...
  stats: Pick<
    ...
    | 'v2PatchAttempted'
    | 'v2RectsScanned'
    | 'v2RawCandidatesGenerated'
    | 'v2GraphValidCandidates'
    | 'v2SnakeUsableCandidates'
    | 'v2ImmediateLockedCandidates'
    | 'v2NonImmediateCandidates'
    | 'v2TransitionCandidatesAfterPrefilter'
    | 'v2TransitionCandidatesSkippedByPrefilter'
    | 'v2TransitionSearchesStarted'
    | 'v2TransitionSearchesSucceeded'
    | 'v2TransitionReachableCandidates'
    | 'v2RejectedByLockedCertificate'
    | 'v2ImprovingCandidates'
    | 'v2RejectedNoImprovement'
    | 'v2RejectedBudget'
    | 'v2ImmediateLockedSelectedWithoutTransition'
    | 'bestV2Improvement'
    | 'v2DetectionMs'
  ...
  let classification = classifyGeneratedV2FourExitSpliceCandidatesForSnake(
    state,
  ...
  if (shouldUseBroadFallback(spliceDiagnostics.aggregate.graphValidCandidates, classification.aggregate.snakeUsableCandidates, options)) {
    ...
    classification = classifyGeneratedV2FourExitSpliceCandidatesForSnake(
      state,
      candidatesToClassify,
      currentCycle,
      v2Options
  ...
  const improvingCandidates = classification.rankedCandidates.filter(
    (candidate) => (candidate.features.pathLenImprovement ?? Number.NEGATIVE_INFINITY) >= options.minimumPathImprovement
  );
  let selected: SelectedCertifiedCandidate | null = null;
  ...
  const rectBudgetHit =
    Number.isFinite(options.maxV2RectsScanned) &&
  ...
  return {
    ...
    stats: {
      ...
      headAppleRectangles: targetedRectangles.headAppleRectangles,
      candidateYieldPerRectangle: yieldPerRectangle(
        spliceDiagnostics.aggregate.graphValidCandidates,
        spliceDiagnostics.pathCoverDiagnostics.patchScan.aggregate.rectanglesScanned
      ),
      improvingYieldPerRectangle: yieldPerRectangle(
        classification.aggregate.improvingCandidates,
        spliceDiagnostics.pathCoverDiagnostics.patchScan.aggregate.rectanglesScanned
      )
    }
  };
...
function betterSelectedCandidate(
  current: SelectedCertifiedCandidate | null,
  candidate: SelectedCertifiedCandidate | null
): SelectedCertifiedCandidate | null {
  if (!candidate) {
    return current;
  }
  if (!current) {
...
function sourcePrecedence(source: CertifiedSelectedCandidateSource): number {
  switch (source) {
    case 'library':
      return 0;
    case 'transition':
...
function evaluateSwitchableCandidates(
  state: GameState,
  cycleLibrary: CycleLibrary,
  options: Required<CertifiedCycleSelectionOptions>
): {
  selected: SelectedCertifiedCandidate | null;
  stats: CertifiedSwitchSelectionStats;
} {
  ...
  const proofPassingEntries: CycleLibraryEntry[] = [];
  const proofPassingCandidateDiagnostics: Array<{
    entry: CycleLibraryEntry;
  ...
  for (const entry of cycleLibrary.entries) {
    ...
    const pathLenDelta =
      currentPathLen !== null && features.pathLen !== null
  ...
  const librarySelectedDiagnostic = librarySelected
    ...
    : null;
  const selectedLibraryCandidate: SelectedCertifiedCandidate | null = librarySelected
    ? {
        source: 'library',
  ...
  stats.candidateYieldPerRectangle = yieldPerRectangle(
    stats.patchGraphValidCandidates + stats.v2GraphValidCandidates,
    patchRectanglesEvaluated
  );
  stats.improvingYieldPerRectangle = yieldPerRectangle(
    stats.patchImprovingCandidates + stats.v2ImprovingCandidates,
    patchRectanglesEvaluated
  );

  const selected = betterSelectedCandidate(
    betterSelectedCandidate(selectedLibraryCandidate, patchSelection?.selected ?? null),
    v2PatchSelection?.selected ?? null
  );
  ...
  let finalDecisionReason: CertifiedSwitchDecisionReason = selected ? 'selected-switch' : 'no-proof-valid-candidates';
  if (!selected && (proofPassingCandidateDiagnostics.length > 0 || stats.patchSnakeUsableCandidates > 0 || stats.v2SnakeUsableCandidates > 0)) {
    const improvingCandidates = proofPassingCandidateDiagnostics.filter(
      (candidate) => candidate.pathLenDelta !== null && candidate.pathLenDelta >= options.minimumPathImprovement
    );
  ...
  stats.switchAttemptSummary = {
    ...
    proofValidCandidates: proofPassingCandidateDiagnostics.map((candidate) => {
      const rejectedBecauseNotImprovingPathLen =
        candidate.pathLenDelta === null || candidate.pathLenDelta < options.minimumPathImprovement;
      const rejectedBecauseScoreComparatorWorse =
...
function mergeRuntimeSwitchingDiagnostics(
  ...
  diagnostics.candidateYieldPerRectangle = yieldPerRectangle(
    diagnostics.patchGraphValidCandidates + diagnostics.v2GraphValidCandidates,
    diagnostics.patchRectsScanned + diagnostics.v2RectsScanned
  );
  diagnostics.improvingYieldPerRectangle = yieldPerRectangle(
    diagnostics.patchImprovingCandidates + diagnostics.v2ImprovingCandidates,
    diagnostics.patchRectsScanned + diagnostics.v2RectsScanned
  );
...
export function selectBestSwitchableCycle(
  state: GameState,
  cycleLibrary: CycleLibrary,
  options: CertifiedCycleSelectionOptions = {}
): CycleLibraryEntry | null {
...
export function debugCertifiedSwitchSelection(
  state: GameState,
  cycleLibrary: CycleLibrary,
  options: CertifiedCycleSelectionOptions = {}
): CertifiedSwitchDebugSnapshot | null {
  if (cycleLibrary.status !== 'ready' || state.appleNodeId === null) {
    return null;
  }
  ...
  const candidates = cycleLibrary.entries.map((entry) => {
    if (isSameAsLockedCycle(state, entry)) {
      return {
        ...
        pathLenDelta: null,
        scoreDelta: null,
        finalDecision: 'skipped-current-cycle' as const
      };
    }
    ...
    const appleForwardValid =
      graphValid && lockedCertificateValid && state.appleNodeId !== null
    ...
    let pathLenDelta: number | null = null;
    ...
    let finalDecision: CertifiedCandidateDecision;
    if (!graphValid) {
...
function stateWithTransitionPlan(state: GameState, plan: CertifiedTransitionPlan): GameState {
  return {
    ...state,
...
function ensureValidTransitionPlanTargetOrThrow(state: GameState, plan: CertifiedTransitionPlan): void {
  if (!validateHamiltonianCycle(state.map.graph, plan.targetCycle)) {
    certifiedInvariantError(
      `Certified Hamiltonian AI invariant failed: transition target cycle ${plan.targetCycleId} is not graph-valid.`
    );
  }
}

function progressCertifiedTransitionPlan(
  previousState: GameState,
  nextState: GameState,
  plan: CertifiedTransitionPlan
): GameState {
  ...
  if (nextState.applesEaten > previousState.applesEaten) {
    if (!validLockedCertificate(nextState.snake.segments, plan.targetCycle)) {
      certifiedInvariantError(
        `Certified Hamiltonian AI invariant failed: transition target cycle ${plan.targetCycleId} does not certify the real post-apple body.`
      );
    }
...
export function setCertifiedLockedCycleOrThrow(
  state: GameState,
  cycle: HamiltonianCycle,
  cycleId: string | null
): GameState {
  if (!validateHamiltonianCycle(state.map.graph, cycle)) {
    certifiedInvariantError(
      `Certified Hamiltonian AI invariant failed: locked cycle ${cycleId ?? 'unknown'} does not form a valid Hamiltonian cycle for the current map graph.`
    );
  }

  if (!validLockedCertificate(state.snake.segments, cycle)) {
    certifiedInvariantError(
      `Certified Hamiltonian AI invariant failed: locked cycle ${cycleId ?? 'unknown'} does not satisfy the locked Hamiltonian certificate for the current body.`
    );
  }
...
export interface CertifiedPostAppleTransitionArgs {
  previousState: GameState;
  nextState: GameState;
  cycleLibrary: CycleLibrary | null;
  options?: CertifiedCycleSelectionOptions;
  diagnostics?: CertifiedRuntimeSwitchingDiagnostics | null;
}

function emptyReadyCycleLibrary(mapId: string): CycleLibrary {
  return {
    mapId,
    status: 'ready',
    entries: [],
...
export function applyCertifiedPostAppleTransition({
  previousState,
  nextState,
  cycleLibrary,
  options = {},
  diagnostics = null
}: CertifiedPostAppleTransitionArgs): GameState {
  ...
  if (!selectedCycle) {
    if (diagnostics) {
      diagnostics.oldCycleKept += 1;
      if (
        !selection.stats.hadAnyProofPassingCandidate &&
  ...
  if (selectedCycle.transitionPlan) {
    return stateWithTransitionPlan(
      setCertifiedLockedCycleOrThrow(stateOnOldCycle, oldCycle, oldCycleId),
      selectedCycle.transitionPlan
    );
  }
...
export function describeCertifiedLibraryStatus(state: GameState, cycleLibrary: CycleLibrary): string {
...
export function strictCertifiedDecision(state: GameState): AiDecision | null {
...
export function currentCyclePathLen(state: GameState): number | null {
  const head = state.snake.segments[0];
