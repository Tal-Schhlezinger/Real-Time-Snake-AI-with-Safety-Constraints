...
const hydratedGraphCache = new WeakMap<SavedMap, ReturnType<typeof hydrateGraph>>();

function getHydratedGraph(map: SavedMap): ReturnType<typeof hydrateGraph> {
...
function classifyMissingEdge(map: SavedMap, fromNodeId: NodeId, direction: Direction): DeathReason {
...
export function listPotentialMoves(state: GameState): MoveResolution[] {
...
export function isMoveImmediatelySafe(state: GameState, direction: Direction): boolean {
...
export function listSafeDirections(state: GameState): Direction[] {
...
export function simulateDirections(state: GameState, directions: Direction[]): GameState {
  let current = state;
