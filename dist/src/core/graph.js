...
export function buildGraphFromDraft(draft) {
...
export function hydrateGraph(snapshot) {
...
export function compareGraphSnapshots(a, b) {
...
export function edgeExists(graph, from, to) {
    return (graph.outgoing.get(from) ?? []).some((edge) => edge.to === to);
}
export function graphNodeIds(snapshot) {
