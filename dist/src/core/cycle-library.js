...
import { solveHamiltonianCycle } from './hamiltonian-cycle-solver.js';
import { validateHamiltonianCycle } from './map-validator.js';
import { generateHorizontalSerpentineCycle, generateVerticalSerpentineCycle, transformRectangleCycle } from './rectangular-cycle.js';
...
export function createBaseCycleLibraryEntryId(mapId) {
    return `${mapId}:base`;
}
function createCycleEntryId(mapId, source, suffix) {
    return `${mapId}:${source}:${suffix}`;
}
export function cycleEdgeSignature(cycle) {
    const edges = [];
...
export function cycleDistance(left, right) {
...
export function cycleOrderDistance(left, right) {
    if (left.length === 0 || left.length !== right.length) {
        return 1;
    }
...
export function isCycleLibrarySupportedMap(map) {
...
export function generateDiverseHamiltonianCycles(map, options = {}) {
    const resolved = {
