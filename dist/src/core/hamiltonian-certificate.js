...
export function cycleIndexOf(cell, cycle) {
...
export function distanceForwardOnCycle(from, to, cycle) {
    const cycleIndexByNode = buildCycleIndexMap(cycle);
    if (!cycleIndexByNode) {
        return null;
    }
...
export function getCertifiedHamiltonianDebugInfo(state) {
...
export function bodyContiguous(bodySegments, cycle) {
...
export function explainLockedCertificateFailure(bodySegments, cycle) {
    if (bodySegments.length === 0 || cycle.length === 0 || bodySegments.length > cycle.length) {
