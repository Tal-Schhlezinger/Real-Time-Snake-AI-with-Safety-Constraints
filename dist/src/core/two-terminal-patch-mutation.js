import { buildRectanglePathCache, localCoord, localIndex } from './rectangle-path-cache.js';
import { validateHamiltonianCycle } from './map-validator.js';
import { analyzeCertifiedTransitionTargets } from './certified-transition-diagnostics.js';
import { appleForward, distanceForwardOnCycle, validLockedCertificate } from './hamiltonian-certificate.js';
import { computeCycleFeatures, scoreCycleFeatures } from './cycle-scoring.js';
...
export function sameNodeSet(pathA, pathB) {
...
export function pathUsesExactlyPatch(path, patchNodeSet) {
...
export function extractCycleSegment(oldCycle, terminalA, terminalB, patchNodeSet) {
...
export function enumerateRectangles(graph, options = {}) {
    const resolved = {
...
export function isFullRectangleInBoard(graph, rect) {
    return buildRectNodeSet(graph, rect).size === rect.width * rect.height;
}
export function getCycleCutCrossings(cycle, rectNodeSet) {
    const crossings = [];
...
export function extractInsideCycleEdges(cycle, rectNodeSet) {
    const edges = [];
    for (let index = 0; index < cycle.length; index += 1) {
        const from = cycle[index];
        const to = cycle[(index + 1) % cycle.length];
...
export function extractInsideCyclePath(cycle, rectNodeSet, terminalA, terminalB) {
    return extractCycleSegment(cycle, terminalA, terminalB, rectNodeSet);
}
export function analyzeRectanglePatch(graph, cycle, rect, rectanglePathCache = buildRectanglePathCache()) {
...
export function analyzeRectanglePatches(graph, cycle, options = {}) {
...
export function generateRectanglePatchMutationCandidates(graph, cycle, options = {}) {
...
function classifyPatchMutationCandidateForSnakeWithTiming(state, candidate, options = {}) {
    ...
    const transitionDiagnostics = analyzeCertifiedTransitionTargets(state, {
        mapId: state.map.id,
        status: 'ready',
        entries: [{
                id: targetCycleId,
...
export function classifyPatchMutationCandidateForSnake(state, candidate, options = {}) {
...
export function classifyPatchMutationCandidatesForSnake(state, graph, cycle, options = {}) {
...
export function classifyGeneratedPatchMutationCandidatesForSnake(state, mutationDiagnostics, candidates, options = {}) {
    const validateCycle = options.validateCycle ?? validateHamiltonianCycle;
...
export function computePatchMutationFeatures(state, currentCycle, classification, candidateIndex = 0) {
    if (!classification.usableForSnake) {
        return null;
    }
...
export function scorePatchMutationCandidate(features) {
...
export function comparePatchMutationFeaturesForRanking(left, right) {
...
export function rankPatchMutationCandidates(state, graph, cycle, options = {}) {
...
export function rankGeneratedPatchMutationCandidates(state, cycle, classificationDiagnostics) {
...
export function spliceTwoTerminalPatchPath(input) {
