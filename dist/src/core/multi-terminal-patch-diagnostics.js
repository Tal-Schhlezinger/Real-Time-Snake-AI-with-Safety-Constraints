...
import { enumerateRectangles, extractInsideCycleEdges, getCycleCutCrossings } from './two-terminal-patch-mutation.js';
...
export function analyzeMultiTerminalRectanglePatch(graph, cycle, rect) {
    ...
    const decompositionResult = analyzeFourExitDecomposition(rectNodeSet, terminals, crossingEdges, extractInsideCycleEdges(cycle, rectNodeSet));
    return {
        ...countedBase,
...
export function analyzeFourExitDecomposition(rectNodeSet, terminals, crossingEdges, internalEdges) {
...
export function analyzeMultiTerminalRectanglePatches(graph, cycle, options = {}) {
...
export function analyzeMultiTerminalRectanglePatchesForRectangles(graph, cycle, rectangles) {
...
export function generateSamePairing4ExitPathCovers(patch, graph, options = {}) {
...
export function validateSamePairingPathCover(patch, graph, cover) {
    const terminalPairs = patch.fourExitDecomposition?.terminalPairs;
    if (patch.rejectionReason !== 'valid-four-exit-decomposition' ||
        patch.exitClass !== 'four' ||
        !terminalPairs ||
...
export function pathCoverSignature(cover) {
    return edgeSignatureForPaths(cover.paths);
}
export function sameAsOriginalCover(patch, cover) {
    return pathCoverSignature(cover) === originalCoverSignature(patch);
}
export function analyzeSamePairing4ExitPathCovers(graph, cycle, options = {}) {
...
export function analyzeSamePairing4ExitPathCoversForRectangles(graph, cycle, rectangles, options = {}) {
    const detectionStartedAt = Date.now();
...
export function spliceMultiTerminalSamePairingCoverByEdges(graph, oldCycle, patch, cover) {
    return spliceMultiTerminalSamePairingCoverByEdgesDetailed(graph, oldCycle, patch, cover).candidateCycle;
}
export function reconstructCycleFromDegreeTwoEdges(edgeSet, startNode) {
    if (edgeSet.length === 0) {
        return null;
    }
...
export function generateV2FourExitSpliceCandidates(graph, cycle, options = {}) {
...
export function generateV2FourExitSpliceCandidatesFromRectangles(graph, cycle, rectangles, options = {}) {
    const startedAt = Date.now();
...
export function classifyV2FourExitSpliceCandidateForSnake(state, candidate, options = {}) {
...
export function classifyV2FourExitSpliceCandidatesForSnake(state, graph, cycle, options = {}) {
...
export function classifyGeneratedV2FourExitSpliceCandidatesForSnake(state, candidates, currentCycle, options) {
    const validateCycle = options.validateCycle ?? validateHamiltonianCycle;
...
export function computeV2FourExitMutationFeatures(state, currentCycle, classification, candidateIndex = 0) {
...
export function scoreV2FourExitMutationCandidate(features) {
...
export function compareV2FourExitMutationFeaturesForRanking(left, right) {
