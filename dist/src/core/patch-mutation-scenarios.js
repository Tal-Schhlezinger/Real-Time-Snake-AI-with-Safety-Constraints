...
import { rankPatchMutationCandidates } from './two-terminal-patch-mutation.js';
...
export function analyzePatchMutationScenarios(map, options = {}) {
...
export function createPatchMutationScenarioStates(map, options = {}) {
    const seedValues = options.seedValues ?? DEFAULT_SEED_VALUES;
    const midGameFillRatios = options.midGameFillRatios ?? DEFAULT_MID_GAME_FILL_RATIOS;
    const scenarios = [];
...
export function analyzePatchMutationScenario(scenario, options = {}) {
    const topCandidateCount = options.topCandidateCount ?? DEFAULT_TOP_CANDIDATE_COUNT;
