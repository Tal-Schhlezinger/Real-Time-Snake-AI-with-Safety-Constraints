...
import { classifyGeneratedV2FourExitSpliceCandidatesForSnake, generateV2FourExitSpliceCandidatesFromRectangles } from './multi-terminal-patch-diagnostics.js';
import { buildRectanglePathCache, localIndex } from './rectangle-path-cache.js';
import { analyzeRectanglePatch, classifyGeneratedPatchMutationCandidatesForSnake, enumerateRectangles, getCycleCutCrossings, rankGeneratedPatchMutationCandidates, spliceTwoTerminalPatchPath } from './two-terminal-patch-mutation.js';
...
export function analyzeHeadAppleRectangleGrowSearch(state, options = {}) {
...
export function generateTargetedRectangles(state, mode, options = {}) {
    const cycle = getLockedCycle(state);
    const arc = getHeadToAppleArc(state, cycle);
...
function analyzeMode(state, cycle, arc, arcSet, mode, options) {
    ...
    const v1Generation = includeV1
        ...
        : emptyV1Generation();
    const v1Classification = classifyGeneratedPatchMutationCandidatesForSnake(state, v1Generation, v1Generation.candidates, options);
    ...
    const v2Classification = v2Generation
        ? classifyGeneratedV2FourExitSpliceCandidatesForSnake(state, v2Generation.candidates.slice(0, maxPatchCandidates), cycle, options)
        : null;
...
export function generateRectanglePatchMutationCandidatesFromRectangles(graph, cycle, rectangles, options) {
