...
exports.AppleForward = exports.ValidLockedCertificate = exports.BodyContiguous = void 0;
exports.cycleIndexOf = cycleIndexOf;
exports.distanceForwardOnCycle = distanceForwardOnCycle;
exports.getCertifiedHamiltonianDebugInfo = getCertifiedHamiltonianDebugInfo;
exports.bodyContiguous = bodyContiguous;
exports.explainLockedCertificateFailure = explainLockedCertificateFailure;
exports.validLockedCertificate = validLockedCertificate;
exports.appleForward = appleForward;
...
function cycleIndexOf(cell, cycle) {
...
function distanceForwardOnCycle(from, to, cycle) {
    const cycleIndexByNode = buildCycleIndexMap(cycle);
    if (!cycleIndexByNode) {
        return null;
    }
...
function getCertifiedHamiltonianDebugInfo(state) {
    ...
    const distanceHeadToApple = head && state.appleNodeId ? distanceForwardOnCycle(head, state.appleNodeId, cycle) : null;
    return {
        headIndex,
        tailIndex,
        appleIndex,
...
function bodyContiguous(bodySegments, cycle) {
...
exports.BodyContiguous = bodyContiguous;
function explainLockedCertificateFailure(bodySegments, cycle) {
    if (bodySegments.length === 0 || cycle.length === 0 || bodySegments.length > cycle.length) {
...
function validLockedCertificate(bodySegments, cycle) {
    return explainLockedCertificateFailure(bodySegments, cycle) === null;
}
exports.ValidLockedCertificate = validLockedCertificate;
function appleForward(bodySegments, apple, cycle) {
    if (!apple) {
        return false;
    }
