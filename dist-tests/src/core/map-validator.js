...
exports.validateHamiltonianCycle = validateHamiltonianCycle;
exports.validateDraftMap = validateDraftMap;
exports.validateLoadedMap = validateLoadedMap;
...
function walkConnectivity(graph) {
    const nodeIds = [...graph.nodesById.keys()];
    const firstNodeId = nodeIds[0];
    if (!firstNodeId) {
        return new Set();
    }
    const seen = new Set([firstNodeId]);
...
function findBridgeEdges(graph) {
    const nodeIds = [...graph.nodesById.keys()];
...
function validateHamiltonianCycle(graphSnapshot, cycle) {
    if (cycle.length !== graphSnapshot.nodes.length) {
        return false;
    }
    const graph = (0, graph_js_1.hydrateGraph)(graphSnapshot);
    ...
    for (let index = 0; index < cycle.length; index += 1) {
        ...
        if (!(0, graph_js_1.edgeExists)(graph, current, next)) {
            return false;
        }
    }
    return true;
}
function validateDraftMap(draft, options = {}) {
    ...
    for (const portal of draft.portals) {
        const aKey = (0, coords_js_1.coordKey)(portal.a);
        const bKey = (0, coords_js_1.coordKey)(portal.b);
    ...
    const spawnKey = (0, coords_js_1.coordKey)(draft.snakeSpawn);
    ...
    const { graph, portalErrors } = (0, graph_js_1.buildGraphFromDraft)(draft);
    ...
    const graphView = (0, graph_js_1.hydrateGraph)(graph);
    const spawnNodeId = (0, coords_js_1.nodeIdForCoord)(draft.snakeSpawn);
    ...
    const solved = (0, hamiltonian_cycle_solver_js_1.solveHamiltonianCycle)(graph, options);
...
function validateLoadedMap(savedMap, options = {}) {
    const startedAt = performance.now();
    const draft = {
        id: savedMap.id,
        name: savedMap.name,
    ...
    if (wallSet.has((0, coords_js_1.coordKey)(draft.snakeSpawn)) || portalCells.has((0, coords_js_1.coordKey)(draft.snakeSpawn))) {
        return {
            isValid: false,
            graph: { nodes: [], edges: [] },
            cycle: [],
    ...
    const { graph, portalErrors } = (0, graph_js_1.buildGraphFromDraft)(savedMap);
    if (portalErrors.length > 0) {
        return {
            isValid: false,
            graph,
    ...
    if (!(0, graph_js_1.compareGraphSnapshots)(graph, savedMap.graph) || !validateHamiltonianCycle(graph, savedMap.hamiltonianCycle)) {
        return {
            isValid: false,
            graph,
            cycle: [],
