...
exports.CanSwitchAndLock = void 0;
exports.createCertifiedRuntimeSwitchingDiagnostics = createCertifiedRuntimeSwitchingDiagnostics;
exports.getCertifiedLockedCycle = getCertifiedLockedCycle;
exports.canSwitchAndLock = canSwitchAndLock;
exports.selectBestSwitchableCycle = selectBestSwitchableCycle;
exports.debugCertifiedSwitchSelection = debugCertifiedSwitchSelection;
exports.setCertifiedLockedCycleOrThrow = setCertifiedLockedCycleOrThrow;
exports.applyCertifiedPostAppleTransition = applyCertifiedPostAppleTransition;
exports.describeCertifiedLibraryStatus = describeCertifiedLibraryStatus;
exports.strictCertifiedDecision = strictCertifiedDecision;
exports.currentCyclePathLen = currentCyclePathLen;
...
const patch_mutation_candidate_cache_js_1 = require("./patch-mutation-candidate-cache.js");
...
function createCertifiedRuntimeSwitchingDiagnostics() {
    return {
        applesEaten: 0,
        switchAttempts: 0,
...
const DEFAULT_CERTIFIED_CYCLE_SELECTION_OPTIONS = {
    ...
    patchMutationCandidateCache: patch_mutation_candidate_cache_js_1.defaultPatchMutationCandidateCache,
...
function certifiedInvariantError(message) {
    throw new certified_hamiltonian_error_js_1.CertifiedHamiltonianInvariantError(message);
}
...
function getStrictCycleDirection(state, cycle) {
    ...
    return (0, game_engine_js_1.listPotentialMoves)(state).find((candidate) => candidate.to === successor)?.direction ?? null;
}
function currentLockedCycleSignature(state) {
    return (0, cycle_library_js_1.cycleEdgeSignature)(getCertifiedLockedCycle(state));
}
function isSameAsLockedCycle(state, entry) {
    if (state.lockedHamiltonianCycleId && state.lockedHamiltonianCycleId === entry.id) {
        return true;
    }
    return currentLockedCycleSignature(state) === (0, cycle_library_js_1.cycleEdgeSignature)(entry.cycle);
}
function canSwitchAndLock(state, candidateEntry) {
    if (!(0, map_validator_js_1.validateHamiltonianCycle)(state.map.graph, candidateEntry.cycle)) {
        return false;
    }
    if (!(0, hamiltonian_certificate_js_1.validLockedCertificate)(state.snake.segments, candidateEntry.cycle)) {
        return false;
    }
    if (state.appleNodeId !== null && !(0, hamiltonian_certificate_js_1.appleForward)(state.snake.segments, state.appleNodeId, candidateEntry.cycle)) {
        return false;
    }
    return true;
}
...
function rankCandidateEntries(state, currentCycle, candidates, options) {
    const baselineFeatures = (0, cycle_scoring_js_1.computeCycleFeatures)(state, currentCycle, currentCycle);
    ...
    const accepted = candidates
        .map((entry) => ({
        entry,
        features: (0, cycle_scoring_js_1.computeCycleFeatures)(state, currentCycle, entry.cycle)
    }))
    ...
    accepted.sort((left, right) => {
        ...
        return (0, cycle_scoring_js_1.compareCandidateCycles)(left.features, right.features, options.scoreWeights);
    });
...
function buildExpectedHeadPathForTransition(state, directions) {
    ...
    for (const direction of directions) {
        current = (0, game_engine_js_1.advanceGame)(current, direction, 0, { next: () => 0 });
...
function rankV1PatchCandidatesForRectangles(state, currentCycle, classificationOptions, options, rectangles) {
    const cachedGeneration = options.enablePatchMutationCandidateCache
        ? (0, patch_mutation_candidate_cache_js_1.getOrCreateV1GraphCandidates)({
    ...
    const generated = cachedGeneration?.result ?? (rectangles
        ? (0, head_apple_rectangle_diagnostics_js_1.generateRectanglePatchMutationCandidatesFromRectangles)(state.map.graph, currentCycle, rectangles, classificationOptions)
        : null);
    const ranking = generated
        ? (0, two_terminal_patch_mutation_js_1.rankGeneratedPatchMutationCandidates)(state, currentCycle, (0, two_terminal_patch_mutation_js_1.classifyGeneratedPatchMutationCandidatesForSnake)(state, generated, generated.candidates, classificationOptions))
        : (0, two_terminal_patch_mutation_js_1.rankPatchMutationCandidates)(state, state.map.graph, currentCycle, classificationOptions);
    return {
        ...ranking,
...
function generateV2PatchCandidatesForRectangles(state, currentCycle, v2Options, options, rectangles) {
    const cachedGeneration = options.enablePatchMutationCandidateCache
        ? (0, patch_mutation_candidate_cache_js_1.getOrCreateV2GraphCandidates)({
            cache: options.patchMutationCandidateCache,
            mapId: state.map.id,
            graph: state.map.graph,
            cycle: currentCycle,
    ...
    return {
        spliceDiagnostics: cachedGeneration?.result ?? (rectangles
            ? (0, multi_terminal_patch_diagnostics_js_1.generateV2FourExitSpliceCandidatesFromRectangles)(state.map.graph, currentCycle, rectangles, v2Options)
            : (0, multi_terminal_patch_diagnostics_js_1.generateV2FourExitSpliceCandidates)(state.map.graph, currentCycle, v2Options)),
        cacheHit: cachedGeneration?.cacheHit ?? false,
        cacheMiss: Boolean(cachedGeneration && !cachedGeneration.cacheHit)
    };
}
function buildRuntimeTargetedRectangles(state, options, mode) {
    ...
    const rectangles = (0, head_apple_rectangle_diagnostics_js_1.generateTargetedRectangles)(state, targetedMode, rectangleOptions);
    const arcChunkRectangles = targetedMode === 'arc-chunk' || targetedMode === 'combined-targeted'
        ? (0, head_apple_rectangle_diagnostics_js_1.generateTargetedRectangles)(state, 'arc-chunk', rectangleOptions).length
        : 0;
    const headAppleRectangles = targetedMode === 'head-apple' || targetedMode === 'combined-targeted'
        ? (0, head_apple_rectangle_diagnostics_js_1.generateTargetedRectangles)(state, 'head-apple', rectangleOptions).length
        : 0;
    return {
        rectangles,
        arcChunkRectangles,
...
function selectBestV2PatchMutationCandidate(state, currentCycle, options) {
    ...
    let classification = (0, multi_terminal_patch_diagnostics_js_1.classifyGeneratedV2FourExitSpliceCandidatesForSnake)(state, candidatesToClassify, currentCycle, v2Options);
    if (shouldUseBroadFallback(spliceDiagnostics.aggregate.graphValidCandidates, classification.aggregate.snakeUsableCandidates, options)) {
        ...
        classification = (0, multi_terminal_patch_diagnostics_js_1.classifyGeneratedV2FourExitSpliceCandidatesForSnake)(state, candidatesToClassify, currentCycle, v2Options);
    }
...
function evaluateSwitchableCandidates(state, cycleLibrary, options) {
    ...
    const currentFeatures = (0, cycle_scoring_js_1.computeCycleFeatures)(state, currentCycle, currentCycle);
    const currentPathLen = currentFeatures.pathLen;
    const currentScore = (0, cycle_scoring_js_1.scoreCycleFeatures)(currentFeatures, options.scoreWeights);
    ...
    for (const entry of cycleLibrary.entries) {
        ...
        if (!(0, map_validator_js_1.validateHamiltonianCycle)(state.map.graph, entry.cycle)) {
            stats.candidatesRejectedByGraphValidity += 1;
            continue;
        }
        stats.candidatesPassingGraphValidity += 1;
        if (!(0, hamiltonian_certificate_js_1.validLockedCertificate)(state.snake.segments, entry.cycle)) {
            stats.candidatesRejectedByLockedCertificate += 1;
            continue;
        }
        stats.candidatesPassingLockedCertificate += 1;
        if (state.appleNodeId !== null && !(0, hamiltonian_certificate_js_1.appleForward)(state.snake.segments, state.appleNodeId, entry.cycle)) {
        ...
        const features = (0, cycle_scoring_js_1.computeCycleFeatures)(state, currentCycle, entry.cycle);
        const score = (0, cycle_scoring_js_1.scoreCycleFeatures)(features, options.scoreWeights);
...
function selectBestSwitchableCycle(state, cycleLibrary, options = {}) {
...
function debugCertifiedSwitchSelection(state, cycleLibrary, options = {}) {
    if (cycleLibrary.status !== 'ready' || state.appleNodeId === null) {
        return null;
    }
    const resolved = {
    ...
    const currentFeatures = (0, cycle_scoring_js_1.computeCycleFeatures)(state, currentCycle, currentCycle);
    const currentPathLen = currentFeatures.pathLen;
    const currentScore = (0, cycle_scoring_js_1.scoreCycleFeatures)(currentFeatures, resolved.scoreWeights);
    const selected = evaluateSwitchableCandidates(state, cycleLibrary, resolved).selected;
    const candidates = cycleLibrary.entries.map((entry) => {
        ...
        const graphValid = (0, map_validator_js_1.validateHamiltonianCycle)(state.map.graph, entry.cycle);
        const lockedCertificateFailure = graphValid ? (0, hamiltonian_certificate_js_1.explainLockedCertificateFailure)(state.snake.segments, entry.cycle) : null;
        ...
        const appleForwardValid = graphValid && lockedCertificateValid && state.appleNodeId !== null
            ? (0, hamiltonian_certificate_js_1.appleForward)(state.snake.segments, state.appleNodeId, entry.cycle)
        ...
        if (graphValid) {
            ...
            score = (0, cycle_scoring_js_1.scoreCycleFeatures)(features, resolved.scoreWeights);
            pathLenDelta =
...
function ensureValidTransitionPlanTargetOrThrow(state, plan) {
    if (!(0, map_validator_js_1.validateHamiltonianCycle)(state.map.graph, plan.targetCycle)) {
...
function progressCertifiedTransitionPlan(previousState, nextState, plan) {
    ...
    if (nextState.applesEaten > previousState.applesEaten) {
        if (!(0, hamiltonian_certificate_js_1.validLockedCertificate)(nextState.snake.segments, plan.targetCycle)) {
...
function setCertifiedLockedCycleOrThrow(state, cycle, cycleId) {
    if (!(0, map_validator_js_1.validateHamiltonianCycle)(state.map.graph, cycle)) {
    ...
    if (!(0, hamiltonian_certificate_js_1.validLockedCertificate)(state.snake.segments, cycle)) {
...
function applyCertifiedPostAppleTransition({ previousState, nextState, cycleLibrary, options = {}, diagnostics = null }) {
    ...
    const certifiedPhase = (0, certified_phase_controller_js_1.selectCertifiedPhase)(nextState);
    const baseState = {
        ...nextState,
        certifiedPhase
    };
...
function describeCertifiedLibraryStatus(state, cycleLibrary) {
...
function strictCertifiedDecision(state) {
...
function currentCyclePathLen(state) {
    const head = state.snake.segments[0];
    const apple = state.appleNodeId;
    if (!head || !apple) {
        return null;
    }
    return (0, hamiltonian_certificate_js_1.distanceForwardOnCycle)(head, apple, getCertifiedLockedCycle(state));
}
