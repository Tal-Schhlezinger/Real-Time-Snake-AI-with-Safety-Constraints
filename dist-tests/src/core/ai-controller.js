...
function getHamiltonianMove(state) {
    ...
    const move = (0, game_engine_js_1.listPotentialMoves)(state).find((candidate) => candidate.to === successor);
    if (!move || !(0, game_engine_js_1.isMoveImmediatelySafe)(state, move.direction)) {
        return null;
    }
    return {
...
function getCertifiedHamiltonianMove(state) {
    ...
    const head = state.snake.segments[0];
    const cycle = (0, certified_cycle_controller_js_1.getCertifiedLockedCycle)(state);
...
function getCertifiedTransitionMove(state) {
    ...
    const move = (0, game_engine_js_1.listPotentialMoves)(state).find((candidate) => candidate.direction === direction);
...
function chooseRoomiestMove(state) {
    const safeDirections = (0, game_engine_js_1.listSafeDirections)(state);
    ...
    for (const direction of safeDirections) {
        const simulated = (0, game_engine_js_1.advanceGame)(state, direction, 0, { next: () => 0 });
        const graph = (0, graph_js_1.hydrateGraph)(simulated.map.graph);
...
function decideAiMove(state, preferredStrategy) {
    ...
    if (state.appleNodeId) {
        ...
        if (pathToApple && pathToApple.length > 0) {
            const simulated = (0, game_engine_js_1.simulateDirections)(state, pathToApple);
