...
exports.collectSpawnableNodeIds = collectSpawnableNodeIds;
exports.spawnAppleNode = spawnAppleNode;
function collectSpawnableNodeIds(graph, occupied) {
    const blocked = new Set(occupied);
    return graph.nodes.map((node) => node.id).filter((nodeId) => !blocked.has(nodeId));
}
function spawnAppleNode(graph, occupied, random = { next: () => Math.random() }) {
