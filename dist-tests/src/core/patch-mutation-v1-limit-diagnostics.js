...
exports.DEFAULT_12X8_PATCH_LIMIT_CONFIGS = void 0;
exports.analyzePatchMutationV1LimitDiagnostics = analyzePatchMutationV1LimitDiagnostics;
exports.analyzePatchMutationV1LimitConfig = analyzePatchMutationV1LimitConfig;
exports.analyzeMultiExitRectangles = analyzeMultiExitRectangles;
...
exports.DEFAULT_12X8_PATCH_LIMIT_CONFIGS = [
    {
        id: 'current-default',
...
function analyzePatchMutationV1LimitDiagnostics(map, options = {}) {
    const scenarios = options.scenarios ?? (0, patch_mutation_scenarios_js_1.createPatchMutationScenarioStates)(map, {
        seedValues: [0, 0.37],
    ...
    const configs = options.configs ?? exports.DEFAULT_12X8_PATCH_LIMIT_CONFIGS;
    return {
        mapId: map.id,
        mapName: map.name,
...
function analyzePatchMutationV1LimitConfig(scenarios, config) {
    ...
    for (const scenario of scenarios) {
        ...
        const ranking = (0, two_terminal_patch_mutation_js_1.rankPatchMutationCandidates)(state, state.map.graph, lockedCycle, resolvedOptions);
...
function analyzeMultiExitRectangles(graph, cycle, options = {}) {
    const diagnostics = createEmptyMultiExitDiagnostics();
    for (const rect of (0, two_terminal_patch_mutation_js_1.enumerateRectangles)(graph, options)) {
...
function hasPlausibleMultiExitInternalDegreePattern(cycle, rectNodeSet) {
    ...
    for (const edge of (0, two_terminal_patch_mutation_js_1.extractInsideCycleEdges)(cycle, rectNodeSet)) {
        insideDegree.set(edge.from, (insideDegree.get(edge.from) ?? 0) + 1);
        insideDegree.set(edge.to, (insideDegree.get(edge.to) ?? 0) + 1);
    }
    for (const crossing of (0, two_terminal_patch_mutation_js_1.getCycleCutCrossings)(cycle, rectNodeSet)) {
