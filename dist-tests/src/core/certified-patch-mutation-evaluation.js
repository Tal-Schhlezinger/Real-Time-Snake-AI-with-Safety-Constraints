...
exports.evaluateCertifiedPatchMutationOnMaps = evaluateCertifiedPatchMutationOnMaps;
exports.evaluateCertifiedPatchMutationOnMap = evaluateCertifiedPatchMutationOnMap;
exports.evaluateCertifiedVariant = evaluateCertifiedVariant;
...
function evaluateCertifiedPatchMutationOnMaps(maps, options = {}) {
    return maps.map((map) => evaluateCertifiedPatchMutationOnMap(map, options));
}
function evaluateCertifiedPatchMutationOnMap(map, options = {}) {
    const variants = [
...
function evaluateCertifiedVariant(map, variant, options = {}) {
    const workingMap = clone(map);
    const maxSteps = options.maxSteps ?? DEFAULT_MAX_STEPS;
    const diagnostics = (0, certified_cycle_controller_js_1.createCertifiedRuntimeSwitchingDiagnostics)();
    const random = createSeededRandom(options.seed ?? 1);
    let state = (0, game_state_js_1.createInitialGameState)(workingMap, 'ai', 'certified-hamiltonian', { next: () => normalizeRandom(options.initialAppleSeed ?? random.next()) });
    const cycleLibrary = variant === 'certified-baseline'
        ? null
        : (0, cycle_library_js_1.generateDiverseHamiltonianCycles)(workingMap, options.cycleLibraryOptions);
    const patchMutationCandidateCache = options.patchOptions?.patchMutationCandidateCache ?? new patch_mutation_candidate_cache_js_1.PatchMutationCandidateCache();
    ...
    for (let step = 0; step < maxSteps && !state.isOver; step += 1) {
        ...
        try {
            const decision = (0, ai_controller_js_1.decideAiMove)(previousState, 'certified-hamiltonian');
            if (!decision) {
                stoppedReason = 'invariant-failure';
                invariantFailureMessage = 'Certified Hamiltonian AI returned no decision.';
                break;
            }
            const advancedState = (0, game_engine_js_1.advanceGame)(previousState, decision.direction, 0, random);
            stepsSinceApple += 1;
