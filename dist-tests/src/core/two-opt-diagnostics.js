...
exports.TwoOptDiagnosticAnalyzer = void 0;
exports.constructTwoOptCandidate = constructTwoOptCandidate;
...
function constructTwoOptCandidate(oldCycle, pair, mode) {
...
function buildSearchFocus(state, cycle, neighborhood) {
    ...
    const pathLen = (0, hamiltonian_certificate_js_1.distanceForwardOnCycle)(head, apple, cycle);
...
class TwoOptDiagnosticAnalyzer {
    options;
    ...
    analyze(state, oldCycle) {
        ...
        this.lastDiagnostics = {
            ...
            bestScoreBefore: (0, cycle_scoring_js_1.scoreCycleFeatures)(baselineFeatures, this.options.scoreWeights)
        };
        ...
        for (const attempt of this.generateCandidateAttempts(state, oldCycle)) {
            ...
            if (!(0, map_validator_js_1.validateHamiltonianCycle)(state.map.graph, attempt.candidate)) {
            ...
            if (!(0, hamiltonian_certificate_js_1.bodyContiguous)(state.snake.segments, attempt.candidate)) {
                this.lastDiagnostics.bodyContiguousFailed += 1;
                continue;
            }
            if (!(0, hamiltonian_certificate_js_1.appleForward)(state.snake.segments, state.appleNodeId, attempt.candidate)) {
                this.lastDiagnostics.appleForwardFailed += 1;
                continue;
            }
            const features = (0, cycle_scoring_js_1.computeCycleFeatures)(state, oldCycle, attempt.candidate);
            this.lastDiagnostics.validCandidates += 1;
            if (bestFeatures === null || (0, cycle_scoring_js_1.compareCandidateCycles)(features, bestFeatures, this.options.scoreWeights) < 0) {
        ...
        this.lastDiagnostics.bestScoreAfter = bestFeatures ? (0, cycle_scoring_js_1.scoreCycleFeatures)(bestFeatures, this.options.scoreWeights) : null;
        return {
...
exports.TwoOptDiagnosticAnalyzer = TwoOptDiagnosticAnalyzer;
