...
exports.sameNodeSet = sameNodeSet;
exports.pathUsesExactlyPatch = pathUsesExactlyPatch;
exports.extractCycleSegment = extractCycleSegment;
exports.cycleSignature = cycleSignature;
exports.enumerateRectangles = enumerateRectangles;
exports.isFullRectangleInBoard = isFullRectangleInBoard;
exports.getCycleCutCrossings = getCycleCutCrossings;
exports.extractInsideCycleEdges = extractInsideCycleEdges;
exports.extractInsideCyclePath = extractInsideCyclePath;
exports.analyzeRectanglePatch = analyzeRectanglePatch;
exports.analyzeRectanglePatches = analyzeRectanglePatches;
exports.generateRectanglePatchMutationCandidates = generateRectanglePatchMutationCandidates;
exports.classifyPatchMutationCandidateForSnake = classifyPatchMutationCandidateForSnake;
exports.classifyPatchMutationCandidatesForSnake = classifyPatchMutationCandidatesForSnake;
exports.classifyGeneratedPatchMutationCandidatesForSnake = classifyGeneratedPatchMutationCandidatesForSnake;
exports.computePatchMutationFeatures = computePatchMutationFeatures;
exports.scorePatchMutationCandidate = scorePatchMutationCandidate;
exports.comparePatchMutationFeaturesForRanking = comparePatchMutationFeaturesForRanking;
exports.rankPatchMutationCandidates = rankPatchMutationCandidates;
exports.rankGeneratedPatchMutationCandidates = rankGeneratedPatchMutationCandidates;
exports.spliceTwoTerminalPatchPath = spliceTwoTerminalPatchPath;
...
const DEFAULT_RECTANGLE_PATCH_DETECTION_OPTIONS = {
    maxWidth: 6,
    maxHeight: 6,
    maxArea: 20,
...
function sameNodeSet(pathA, pathB) {
...
function pathUsesExactlyPatch(path, patchNodeSet) {
...
function extractCycleSegment(oldCycle, terminalA, terminalB, patchNodeSet) {
...
function enumerateRectangles(graph, options = {}) {
    const resolved = {
...
function rectangleTouchesFocus(rect, focusedCoords, padding) {
...
function isFullRectangleInBoard(graph, rect) {
    return buildRectNodeSet(graph, rect).size === rect.width * rect.height;
}
function getCycleCutCrossings(cycle, rectNodeSet) {
    const crossings = [];
...
function extractInsideCycleEdges(cycle, rectNodeSet) {
    const edges = [];
    for (let index = 0; index < cycle.length; index += 1) {
        const from = cycle[index];
        const to = cycle[(index + 1) % cycle.length];
...
function extractInsideCyclePath(cycle, rectNodeSet, terminalA, terminalB) {
    return extractCycleSegment(cycle, terminalA, terminalB, rectNodeSet);
}
function analyzeRectanglePatch(graph, cycle, rect, rectanglePathCache = (0, rectangle_path_cache_js_1.buildRectanglePathCache)()) {
...
function analyzeRectanglePatches(graph, cycle, options = {}) {
...
function generateRectanglePatchMutationCandidates(graph, cycle, options = {}) {
...
function classifyPatchMutationCandidateForSnakeWithTiming(state, candidate, options = {}) {
    const validateCycle = options.validateCycle ?? map_validator_js_1.validateHamiltonianCycle;
    ...
    const immediateLockedCertificate = (0, hamiltonian_certificate_js_1.validLockedCertificate)(state.snake.segments, candidate.cycle);
    const immediateAppleForward = !state.appleNodeId || (0, hamiltonian_certificate_js_1.appleForward)(state.snake.segments, state.appleNodeId, candidate.cycle);
...
function classifyPatchMutationCandidateForSnake(state, candidate, options = {}) {
    const { transitionSearchMs: _transitionSearchMs, ...classification } = classifyPatchMutationCandidateForSnakeWithTiming(state, candidate, options);
    return classification;
}
function classifyPatchMutationCandidatesForSnake(state, graph, cycle, options = {}) {
...
function classifyGeneratedPatchMutationCandidatesForSnake(state, mutationDiagnostics, candidates, options = {}) {
    const validateCycle = options.validateCycle ?? map_validator_js_1.validateHamiltonianCycle;
    const lockedCycle = state.lockedHamiltonianCycle ?? [];
    const classificationByIndex = new Array(candidates.length);
    const pendingTransitionCandidates = [];
    ...
    for (let index = 0; index < candidates.length; index += 1) {
        ...
        const immediateLockedCertificate = (0, hamiltonian_certificate_js_1.validLockedCertificate)(state.snake.segments, candidate.cycle);
        const immediateAppleForward = !state.appleNodeId || (0, hamiltonian_certificate_js_1.appleForward)(state.snake.segments, state.appleNodeId, candidate.cycle);
        const cheapTransitionFeatures = buildPatchMutationCheapTransitionFeatures(state, lockedCycle, candidate);
        if (immediateLockedCertificate && immediateAppleForward) {
            classificationByIndex[index] = {
                candidate,
                graphValid,
    ...
    const immediateImprovingExists = options.preferImmediateLockedBeforeTransitionSearch === true &&
        classificationByIndex.some((classification) => classification &&
    ...
    const transitionCandidates = selectPatchTransitionCandidatesForSearch(pendingTransitionCandidates, options, immediateImprovingExists);
...
function computePatchMutationFeatures(state, currentCycle, classification, candidateIndex = 0) {
    if (!classification.usableForSnake) {
        return null;
    }
    ...
    const currentLockedCyclePathLen = head && state.appleNodeId
        ...
        : null;
    const usabilityMode = classification.reason === 'transition-valid'
        ? 'transition-valid'
        : 'immediate-locked';
...
function scorePatchMutationCandidate(features) {
...
function comparePatchMutationFeaturesForRanking(left, right) {
...
function rankPatchMutationCandidates(state, graph, cycle, options = {}) {
...
function rankGeneratedPatchMutationCandidates(state, cycle, classificationDiagnostics) {
...
function buildPatchMutationCheapTransitionFeatures(state, currentCycle, candidate) {
    const head = state.snake.segments[0] ?? null;
...
function selectPatchTransitionCandidatesForSearch(candidates, options, immediateImprovingExists) {
...
function comparePatchCheapTransitionCandidates(left, right, mode) {
...
function resolvePatchTransitionSearchLimit(options) {
    const limits = [
...
function patchTransitionCandidateId(candidate) {
    return `patch-mutation-candidate-${candidate.index}`;
}
function computeCheapChangedCycleEdges(currentCycle, candidateCycle) {
...
function buildDirectedCycleEdgeSet(cycle) {
    const edges = new Set();
    for (let index = 0; index < cycle.length; index += 1) {
...
function computeBodyOrderCompatibilityScore(bodySegments, cycle) {
...
function computeNextOnCycleHeadOccupied(bodySegments, cycle) {
...
function computeRectArcRelevance(state, currentCycle, rect) {
    const head = state.snake.segments[0] ?? null;
    const apple = state.appleNodeId;
...
function collectForwardArcNodeIds(head, apple, cycle) {
    const headIndex = cycle.indexOf(head);
...
function spliceTwoTerminalPatchPath(input) {
    const { oldCycle, patchNodeSet, terminalA, terminalB, oldInternalPath, replacementInternalPath } = input;
...
function orientReplacementToOldPath(oldInternalPath, replacementInternalPath) {
...
function hasTerminalPair(path, terminalA, terminalB) {
    const start = path[0];
    const end = path[path.length - 1];
    return (start === terminalA && end === terminalB) || (start === terminalB && end === terminalA);
}
function findExactCyclePathStart(cycle, path) {
...
function cycleSegmentInclusive(cycle, startIndex, endIndex) {
    const segment = [];
...
function rotateCycle(cycle, startIndex) {
...
function directedCycleEdgeSignature(cycle) {
    const edges = [];
    for (let index = 0; index < cycle.length; index += 1) {
...
function compareRankedPatchMutationCandidates(left, right) {
...
function summarizePatchMutationRanking(rankedCandidates) {
...
function globalPathFromLocalPath(graph, rect, localPath) {
    ...
    for (const localNodeIndex of localPath) {
        const localNodeCoord = (0, rectangle_path_cache_js_1.localCoord)(localNodeIndex, rect.width);
...
function hasValidInternalDegreePattern(rectNodeSet, insideEdges, terminals) {
...
function insideEdgesConnectPatch(rectNodeSet, insideEdges, startNode) {
...
function createEmptyRectanglePatchAggregate() {
    return {
        rectanglesScanned: 0,
        fullRectangles: 0,
...
function createEmptyRectanglePatchMutationAggregate() {
    return {
        patchesScanned: 0,
...
function createEmptySnakePatchMutationClassificationAggregate() {
    return {
        graphValidCandidates: 0,
