...
exports.generateCandidatePathsToApple = generateCandidatePathsToApple;
exports.analyzeCertifiedTransitionTargets = analyzeCertifiedTransitionTargets;
...
function currentLockedSignature(state) {
    return (0, cycle_library_js_1.cycleEdgeSignature)(currentLockedCycle(state));
}
function isCurrentLockedEntry(state, entry) {
    if (state.lockedHamiltonianCycleId && state.lockedHamiltonianCycleId === entry.id) {
        return true;
    }
    return (0, cycle_library_js_1.cycleEdgeSignature)(entry.cycle) === currentLockedSignature(state);
}
...
function sortedOutgoingEdges(state, nodeId) {
    ...
    return [...(graph.outgoing.get(nodeId) ?? [])].sort((left, right) => {
        const directionDifference = coords_js_1.DIRECTIONS.indexOf(left.direction) - coords_js_1.DIRECTIONS.indexOf(right.direction);
...
function generateCandidatePathsToApple(state, options = {}) {
...
function analyzeTargetCycle(state, entry, paths, shortestDistance, budgetExceeded) {
    ...
    for (const path of paths) {
        ...
        safePathsToApple += 1;
        const lockedFailure = (0, hamiltonian_certificate_js_1.explainLockedCertificateFailure)(simulated.state.snake.segments, entry.cycle);
        if (lockedFailure) {
    ...
    const targetFeatures = bestTransitionState
        ? (0, cycle_scoring_js_1.computeCycleFeatures)(bestTransitionState, currentLockedCycle(state), entry.cycle)
        : null;
    return {
        ...
        targetScoreAfterTransition: targetFeatures ? (0, cycle_scoring_js_1.scoreCycleFeatures)(targetFeatures, cycle_scoring_js_1.defaultCycleScoreWeights) : null,
        failureReasons,
        lockedCertificateFailures: [...lockedCertificateFailures]
    };
}
function analyzeCertifiedTransitionTargets(state, cycleLibrary, options = {}) {
