...
import {
  analyzeV2PatchMutationScenario,
  analyzeV2PatchMutationScenarios
} from '../src/core/v2-patch-mutation-scenarios';
...
describe('V2 patch mutation scenario diagnostics', () => {
  it('scenario diagnostics are deterministic for fixed options', () => {
    const map = createRectangularSavedMap({ id: 'v2-scenario-4x4', name: 'V2 Scenario 4x4', width: 4, height: 4 });
    const first = analyzeV2PatchMutationScenarios(map, scenarioOptions);
    const second = analyzeV2PatchMutationScenarios(map, scenarioOptions);
  ...
  it('setting a far apple changes currentPathLen', () => {
    const map = createRectangularSavedMap({ id: 'v2-scenario-4x4', name: 'V2 Scenario 4x4', width: 4, height: 4 });
    const scenarios = analyzeV2PatchMutationScenarios(map, scenarioOptions).scenarios;
  ...
  it('diagnostics include currentPathLen and best improvement', () => {
    const map = createRectangularSavedMap({ id: 'v2-scenario-4x4', name: 'V2 Scenario 4x4', width: 4, height: 4 });
    const scenario = analyzeV2PatchMutationScenarios(map, scenarioOptions).scenarios[0];
    ...
    assert.equal(typeof scenario.v2GraphValidCandidates, 'number');
    assert.equal(typeof scenario.v2SnakeUsableCandidates, 'number');
    assert.equal(typeof scenario.immediateLockedCandidates, 'number');
    assert.equal(typeof scenario.transitionReachableCandidates, 'number');
  ...
  it('diagnostics do not mutate gameplay state', () => {
    ...
    analyzeV2PatchMutationScenario(scenario, scenarioOptions);
    const after = JSON.stringify(scenario.state);

    assert.equal(after, before);
  });

  it('evaluates at least one controlled far-apple scenario', () => {
    const map = createRectangularSavedMap({ id: 'v2-scenario-4x4', name: 'V2 Scenario 4x4', width: 4, height: 4 });
    const report = analyzeV2PatchMutationScenarios(map, scenarioOptions);
  ...
  it('V2 diagnostics do not affect V1 patch mutation scenario diagnostics', () => {
    const map = createRectangularSavedMap({ id: 'v2-scenario-4x4', name: 'V2 Scenario 4x4', width: 4, height: 4 });
    const before = analyzePatchMutationScenarios(map, scenarioOptions);

    analyzeV2PatchMutationScenarios(map, scenarioOptions);

    assert.deepEqual(analyzePatchMutationScenarios(map, scenarioOptions), before);
  });
});
