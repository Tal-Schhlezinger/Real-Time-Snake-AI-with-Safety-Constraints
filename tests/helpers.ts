...
import { selectCertifiedPhase } from '../src/core/certified-phase-controller';
import type { EditableMapDraft, GameState, SavedMap } from '../src/core/types';

export function makeDraft(overrides: Partial<EditableMapDraft>): EditableMapDraft {
  return {
...
export function makeSavedMap(draft: EditableMapDraft, cycle?: string[]): SavedMap {
  const { graph } = buildGraphFromDraft(draft);
  return {
    ...draft,
...
export function makeGameState(overrides: Partial<GameState>): GameState {
  ...
  return {
    ...
    certifiedPhase:
      overrides.certifiedPhase ??
      (overrides.aiStrategy === 'certified-hamiltonian' ? selectCertifiedPhase({
        map: overrides.map,
